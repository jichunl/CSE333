...
// This is the program that connects (via TCP) to the server on the supplied
// hostname and port, reads the bytes from the local file, and writes those
// bytes over the TCP connection. It will close the TCP connection and exit when
// finish sending contents.
...
void HandleClient(int c_fd, struct sockaddr *addr, size_t addrlen,
  ...
  while (1) {
    ...
    // write what it read
    while (wlen != 0) {
      res = fwrite(clientbuf, sizeof(char), wlen, stdout);
      if (res == -1) {
        if ((errno == EINTR) || (errno == EAGAIN)) {
          continue;
        } else {
